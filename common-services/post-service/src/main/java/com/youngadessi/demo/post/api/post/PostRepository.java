...
//    Page<Post> findAllPostByContent(@Param("content") String content, Pageable pageable);
    ...
    Page<customDTO> findAllByContentFromCustomDTO(@Param("content") String content, Pageable pageable);

    @Query(value = "SELECT p.created_by_name AS createdByName, p.content  FROM post_schema.tbl_post p WHERE p.content like %:content%", nativeQuery = true)
    Page<customDTOInterface> findAllByContentFromCustomDTOInterface(@Param("content") String content, Pageable pageable);
